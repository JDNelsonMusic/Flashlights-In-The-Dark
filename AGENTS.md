...
│   ├── choir_onboard.sh              # Shell script for bulk onboarding all connected devices (wraps fastlane, cfgutil, adb)
...
iOS Specifics: The iOS app is generated by Flutter. We have to include the camera usage description in Info.plist (for torch) and microphone usage description. Also, because this is Ad Hoc distribution, all devices must be registered and the app signed accordingly (addressed in the deployment scripts).
...
The app can also initiate device registration by invoking fastlane register_device or building – but those are likely easier via script.
...
Then it calls fastlane register_device with that UDID and name to add it to Apple Dev Portal, and fastlane sync_code_signing to update the provisioning profile. It prints logs accordingly.
Essentially, you would run this script for each new iPhone: plug phone #1, run script with their name, unplug, plug phone #2, etc. Each time it fills the next slot and registers. This is an alternative to the bulk approach.
You’d still need to separately install the app onto those devices. This script doesn’t install the app; it only sets up the provisioning and mapping. Installation can be done via Xcode, TestFlight, or our other script.
...
Registers each via fastlane (calls the same lane as above in a loop).
Refreshes the ad-hoc provisioning profile and then uses cfgutil --install-app to push the pre-built IPA to every connected iPhone.
