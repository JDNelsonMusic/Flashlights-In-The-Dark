...
public final class ConsoleState: ObservableObject, Sendable {
    ...
    public func buildAndRun(device: ChoirDevice) {
        ...
        Task.detached {
            ...
            guard let projectURL = Self.resolveFlutterProjectURL() else {
                await MainActor.run {
                    self.lastLog = "⚠️ Could not locate Software/Singer-Client/ for flutter run"
                }
                return
            }
        ...
        Task.detached {
            guard let projectURL = Self.resolveFlutterProjectURL() else {
                await MainActor.run {
                    ...
                    self.lastLog = "⚠️ Could not locate Software/Singer-Client/ for flutter build"
                }
                return
            }
    ...
    public func run(device: ChoirDevice) {
        ...
        guard let projectURL = Self.resolveFlutterProjectURL() else {
            ...
            lastLog = "⚠️ Could not locate Software/Singer-Client/ for flutter run"
            return
        }
...
extension ConsoleState {
    ...
    nonisolated private static func resolveFlutterProjectURL() -> URL? {
        ...
        let sourceRoot = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent() // Conductor-MacOS
            .deletingLastPathComponent() // Software
        ...
        for candidate in initialCandidates {
            ...
            for _ in 0..<6 {
                let repoCandidate = cursor.appendingPathComponent("Software/Singer-Client", isDirectory: true)
                ...
                if cursor.lastPathComponent == "Singer-Client",
