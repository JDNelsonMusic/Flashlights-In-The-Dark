...
```bash
SIM_DURATION_SEC=300 Operations/Scripts/soak_sim.sh
SIM_QUICK_DURATION_SEC=60 Operations/Scripts/verify.sh
```

Direct simulator invocation:

```bash
python3 Operations/Tools/concert_sim.py --clients 28 --duration-sec 120 --loss-pct 2 --jitter-ms 30 --duplication-pct 3 --reordering-pct 3
```
...
- Legacy onboarding/provisioning scripts remain in `Operations/Scripts/` for historical/debug use and are **deprecated for normal deployment**.
